...
function App() {
  const [activeServer, setActiveServer] = useState<number | undefined>(
    undefined,
  );
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [servers, setServers] = useState<{
    [key: number]: server.Server;
  } | null>(null);
  ...
  const ServerDrawer = (
    <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} size="md">
      ...
      <ServerList
        serverListType={ServerListType.LIST}
        servers={servers}
        setActiveServer={setActiveServer}
        setDrawerOpen={setDrawerOpen}
        handleCreateNewServerClicked={handleCreateNewServerClicked}
      />
      <Divider></Divider>
      <DialogActions>
        <List>
          <ListItem>
            <Tooltip title={"Reload servers from disk"}>
              <IconButton
                color={"danger"}
                variant={"solid"}
                onClick={() => {
            ...
            <Tooltip title={"Refresh server list from cache"}>
              <IconButton
                onClick={() => {
                  getServers();
                  setActiveServer(undefined);
            ...
            <Tooltip title={"Open servers folder"}>
              <IconButton
                onClick={() => {
                  GetServerDir().then((dir: string) =>
    ...
    if (servers !== null && Object.keys(servers).length > 0) {
      ...
      mainUi = (
        <div className={"row-span-5 m-5"}>
          ...
          <Button onClick={() => handleCreateNewServerClicked()}>
            <IconPlus /> Create new server
          </Button>
          ...
          <div className={"ml-auto my-auto mr-8 gap-2 flex"}>
            ...
            <Modal open={infoModalOpen} onClose={() => setInfoModalOpen(false)}>
              <ModalDialog>
                ...
                <Divider />
                <div className={"p-4"}>
                  <Typography level={"title-lg"}>Info:</Typography>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      BrowserOpenURL(
                        "https://github.com/JensvandeWiel/ArkAscendedServerManager",
                      )
                    }
                  >
                    GitHub
                  ...
                  <Button
                    ...
                    sx={{ margin: 1 }}
                    onClick={() =>
                      BrowserOpenURL("https://discord.gg/RmesnZ8FWf")
                    }
                  >
                    Discord
                  </Button>
                  <Button
                    ...
                    onClick={() =>
                      BrowserOpenURL(
                        "https://github.com/sponsors/JensvandeWiel",
                      )
                    }
                  >
                    Sponsor me
