...
function ConsoleCard({setServ, serv, serverStatus, input, messages, terminalRef, setInput, setMessages, writeAndDoRconCommand}: Props & {
    input: string;
    messages: Message[];
...
function CommandCard({setServ, serv, serverStatus, writeAndDoRconCommand}: Props & {writeAndDoRconCommand: (text: string) => void;}) {
    return (
        <Card variant="soft" className={''}>
    ...
    if (serverStatus) {
        return (
            <TabPanel value={0} className={'space-y-8'}>
                <ConsoleCard serv={serv} setServ={setServ} serverStatus={serverStatus} input={input} messages={messages} terminalRef={terminalRef} setInput={setInput} setMessages={setMessages} writeAndDoRconCommand={writeAndDoRconCommand}/>
                <CommandCard serv={serv} setServ={setServ} serverStatus={serverStatus} writeAndDoRconCommand={writeAndDoRconCommand}/>
            </TabPanel>
        );
    } else {
        return (
