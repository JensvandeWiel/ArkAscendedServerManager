...
type Props = {
  ...
  serv: server.Server;
  setGus: React.Dispatch<
    React.SetStateAction<{ [key: string]: { [key: string]: Array<string> } }>
  >;
...
function GeneralSettings({ setServ, serv, setGus, gus }: Props) {
  ...
  if (
    gus === undefined ||
    (Object.keys(gus).length === 1 && Object.keys(gus)[0] === "unknown")
  ) {
    return (
    ...
    return (
      <Card variant="soft" className={""}>
        <Typography level="title-md">Server Name and Passwords</Typography>
        <Divider className={"mx-2"} />
        <div className={"w-[100%] space-y-4"}>
          <div className={""}>
            <FormLabel>Server Name:</FormLabel>
            <Input
              value={serv?.serverName}
              onChange={(e) =>
          ...
          <div className={""}>
            ...
            <PasswordInput
              value={serv?.serverPassword}
              onChange={(e) =>
                setServ((p) => ({ ...p, serverPassword: e.target.value }))
              }
            ></PasswordInput>
          </div>
          <div className={""}>
            ...
            <PasswordInput
              value={serv?.adminPassword}
              onChange={(e) =>
                setServ((p) => ({ ...p, adminPassword: e.target.value }))
              }
            ></PasswordInput>
          </div>
          <div className={""}>
            ...
            <PasswordInput
              value={serv?.spectatorPassword}
              onChange={(e) =>
                setServ((p) => ({ ...p, spectatorPassword: e.target.value }))
              }
            ></PasswordInput>
          </div>
        </div>
        <Typography level="title-md">Server Map</Typography>
        <Divider className={"mx-2"} />
        <div className={"w-[100%] space-y-4"}>
          <div className={""}>
            ...
            <Autocomplete
              freeSolo
              disableClearable
              inputValue={serv?.serverMap}
              options={["TheIsland_WP"]}
              onChange={(e, v) => setServ((p) => ({ ...p, serverMap: v }))}
            ></Autocomplete>
          </div>
        </div>
        <Typography level="title-md">Auto Save</Typography>
        <Divider className={"mx-2"} />
        <div className={"w-[100%] space-y-4"}>
          <div className={""}>
            ...
            <Tooltip title={"Duration that the message is visible in seconds"}>
              <Slider
                value={parseFloat(
                  gus["ServerSettings"]["AutoSavePeriodMinutes"][0] ?? "0",
                )}
                onChange={(v) => {
                  setGus((p) => {
                    const newState = { ...p };
                    newState["ServerSettings"].AutoSavePeriodMinutes[0] =
                      v.toString();
                    return newState;
                  });
                }}
        ...
        <Typography level="title-md">Message of the Day</Typography>
        <Divider className={"mx-2"} />
        <div className={"w-[100%] space-y-4"}>
          <div className={""}>
        ...
        <Typography level="title-md">Server Settings</Typography>
        <Divider className={"mx-2"} />
        <div className={"w-[100%] space-y-4"}>
            ...
            <div className={"flex-grow"}>
              ...
              >
                <FormLabel>
                  ...
                  <Checkbox
                    className={"mr-2"}
                    checked={serv?.kickIdlePlayers}
                    onChange={(e) =>
                      setServ((p) => ({
                        ...p,
                        kickIdlePlayers: e.target.checked,
                      }))
                    }
                  />{" "}
                  Kick Idle Players Period:
                </FormLabel>
              </Tooltip>
              <Slider
                className={""}
                ...
                sliderMax={3600}
                value={
                  parseFloat(gus["ServerSettings"].KickIdlePlayersPeriod[0]) ??
                  0
                }
                onChange={(v) => {
                  if (v >= 0) {
                    setGus((p) => {
                      const newState = { ...p };
                      newState["ServerSettings"].KickIdlePlayersPeriod[0] =
                        v.toString();
                      return newState;
                    });
                  }
...
function NetworkingCard({
  ...
  const [interfaces, setInterfaces] = useState<{ [p: string]: string }>(
    defInter,
  );
  ...
  useEffect(() => {
    GetNetworkInterfacesIp()
      .then((val) => setInterfaces(val))
      .catch((reason) => console.error(reason));
  }, []);

  return (
    <Card variant="soft" className={""}>
      <Typography level="title-md">Networking</Typography>
      <Divider className={"mx-2"} />
      ...
      <div className={"space-x-4 w-11/12"}>
        <div className={"w-32 inline-block"}>
          ...
          <Input
            ...
            type={"number"}
            required
            value={serv?.serverPort}
            onChange={(e) =>
              parseInt(e.target.value) >= 1 && parseInt(e.target.value) <= 65535
                ? setServ((p) => ({
                    ...p,
                    serverPort: parseInt(e.target.value),
                    peerPort: parseInt(e.target.value) + 1,
                  }))
                : null
            }
          ></Input>
        ...
        <div className={"w-32 inline-block"}>
          ...
          <Input
            className={""}
            type={"number"}
            value={serv?.queryPort}
            onChange={(e) =>
              parseInt(e.target.value) >= 1 && parseInt(e.target.value) <= 65535
                ? setServ((p) => ({
                    ...p,
                    queryPort: parseInt(e.target.value),
                  }))
                : null
            }
          ></Input>
        ...
        <div className={"w-32 inline-block"}>
          ...
          <Input
            ...
            onChange={(e) =>
              parseInt(e.target.value) >= 1 && parseInt(e.target.value) <= 65535
                ? setServ((p) => ({ ...p, rconPort: parseInt(e.target.value) }))
                : null
            }
            type={"number"}
          ></Input>
