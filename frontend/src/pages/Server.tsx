...
export const Server = ({ id, className }: Props) => {
  ...
  useEffect(() => {
    if (serv.id >= 0) {
      CheckServerInstalled(serv.id)
        .then((val) => setIsInstalled(val))
        .catch((reason) => console.error(reason));
    }
  }, [serv]);
  ...
  function onServerStartButtonClicked() {
    ...
    if (serv.disableUpdateOnStart) {
      ...
      InstallUpdateVerify(serv.serverPath)
        .catch((err) => {
  ...
  function onServerStopButtonClicked() {
    ...
    StopServer(serv.id)
      .then(() => {
        addAlert("Stopped server", "success");
        setServerStatus(false);
      })
      .catch((err) => addAlert("error stopping server: " + err, "danger"));
  }

  function onServerForceStopButtonClicked() {
    ForceStopServer(serv.id)
      .catch((err) => {
        addAlert(err, "danger");
        console.error(err);
      })
          ...
          <Tabs size="sm" className={"flex h-full w-full overflow-y-auto"}>
            <div className={"h-16 flex w-full"}>
              <div className="flex items-center">
                <Input
                  value={serv?.serverAlias}
                  onChange={(e) =>
                    setServ((p) => ({
                      ...p,
                      serverAlias:
                        e.target.value /*, convertValues: p.convertValues*/,
                    }))
                  }
                />
                <Tooltip title={"Open server install directory"}>
                  <IconButton
                    ...
                    onClick={() => BrowserOpenURL("file:///" + serv.serverPath)}
                  >
                    <IconExternalLink />
                  </IconButton>
                </Tooltip>
              ...
              <div className={"ml-auto my-auto mr-8"}>
                <ButtonGroup aria-label="outlined primary button group">
                  ...
                  <Button
                    ...
                    variant="solid"
                    disabled={!serverStatus}
                    onClick={onServerStopButtonClicked}
                  >
                    Stop
                  </Button>
                  <Button
                    ...
                    variant="solid"
                    disabled={!serverStatus}
                    onClick={() => setForceStopModalOpen(true)}
                  >
                    Force stop
                  </Button>
                </ButtonGroup>
                ...
                <UpdaterModal
                  open={updaterModalOpen}
                  onCompleted={() => setUpdaterModalOpen(false)}
                ></UpdaterModal>
                <Modal
                  open={forceStopModalOpen}
                  onClose={() => setForceStopModalOpen(false)}
                >
                  <ModalDialog variant="outlined" role="alertdialog">
                    ...
                    <Divider />
                    <DialogContent>
                      Are you sure you want to forcefully stop the server? No
                      save action will be performed!
                    </DialogContent>
                    <DialogActions>
                      <Button
                        variant="solid"
                        ...
                        onClick={() => {
                          ...
                          onServerForceStopButtonClicked();
                        }}
                      >
                        Force stop
                      </Button>
                      <Button
                        variant="plain"
                        color="neutral"
                        onClick={() => setForceStopModalOpen(false)}
                      >
                        Cancel
                      </Button>
                    </DialogActions>
                ...
                >
                  <ModalDialog variant="outlined" role="alertdialog">
                    ...
                    <Divider />
                    <DialogContent>
                      Are you sure you want to start the server? This action
                      will overwrite ini files in the server directory!
                      <br />
                    </DialogContent>
                    <DialogActions>
                      <Button
            ...
            <TabList className={"w-full"}>
              <Tab variant="plain" indicatorInset color="neutral">
                Console
              </Tab>
              <Tab variant="plain" indicatorInset color="neutral">
                General Settings
