...
export const Server = ({ id, className }: Props) => {
	...
	useEffect(() => {
		...
		if (
			Object.keys(gus).length === 1 &&
			Object.keys(gus)[0] === "unknown"
		) {
			return;
		}
	...
	function onServerStopButtonClicked() {
		...
		StopServer(serv.id)
			...
			.catch((err) =>
				addAlert("error stopping server: " + err, "danger"),
			);
	}
								...
								<Input
									...
									onChange={(e) =>
										setServ((p) => ({
											...
											serverAlias:
												e.target
													.value /*, convertValues: p.convertValues*/,
										}))
									}
								/>
								<Tooltip
									title={"Open server install directory"}
								>
									<IconButton
										className="text-lg font-bold ml-2"
										onClick={() =>
											BrowserOpenURL(
												"file:///" + serv.serverPath,
											)
										}
									>
										<IconExternalLink />
							...
							<div className={"ml-auto my-auto mr-8"}>
								...
								>
									...
									>
										...
										<Divider />
										<DialogContent>
											Are you sure you want to forcefully
											stop the server? No save action will
											be performed!
										</DialogContent>
										<DialogActions>
											<Button
								...
								>
									...
									>
										...
										<DialogContent>
											...
											server? This action will overwrite
											ini files in the server directory!
											<br />
										</DialogContent>
										<DialogActions>
											<Button
