...
export function InstallUpdater({ setServ, serv, onInstalled }: Props) {
  ...
  function onServerPathClicked() {
    OpenDirectoryDialog().then((val) =>
      setServ((p) => ({ ...p, serverPath: val })),
    );
  }
  ...
  useEffect(() => {
    EventsOn("installingUpdateAction", (data) => {
      setAction(data);
    });
    EventsOn("installingUpdateProgress", (data) => {
      setProgress(data);
    });
    EventsOn("appInstalled", () => {
      setIsCompleted(true);
      setAction("Done");
      setProgress(100);
    });
  ...
  return (
    <div>
      <Modal open={installerModalOpen}>
        <ModalDialog>
          <Typography level="title-md">Install/Update server</Typography>
      ...
      <Card variant="soft" className={""}>
        <Typography level="title-md">Install server</Typography>
        <Divider className={"mx-2"} />
        <FormLabel>Server Path</FormLabel>
        <Input
          className={"w-1/3"}
          value={serv.serverPath}
          required
          onClick={onServerPathClicked}
        ></Input>
        <div className={"text-center pt-5"}>
          <Button
            endDecorator={<IconDownload />}
            onClick={onStartInstallButtonClicked}
            className={"mx-5"}
          >
            Install server
          </Button>
          <Button
            endDecorator={<IconTrash />}
